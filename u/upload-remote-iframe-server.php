...
// $name = $_POST['name'];
    ...
    for($i=0;$i<$countfiles;$i++){
        ...
        if(!empty($ext)){
        $filename1 = 'user-files/' . $_SESSION['id'] . '/'.$_GET['dirName'] . $filename;
...
if (file_exists($filename1)) {
    ...
    //$filename = preg_replace("/[^a-zA-Z]+/", "", $filename);
       ...
       move_uploaded_file($_FILES['file']['tmp_name'][$i], 'user-files/' . $_SESSION['id'] . '/'.$_GET['dirName']. $filename);
       echo '<a target="_blank" href="0/'.$_SESSION['id'].'/'.$filename.'">0/'.$_SESSION['id'].'/'.$_GET['dirName'].$filename.'</a><br>';
